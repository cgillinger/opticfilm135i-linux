...
of it -- see "Split programs" below). The mapping (see `decode_ops`):
  ...
  * `bi`                                              -> `BulkIn`
  * `bo`                                              -> `BulkOut` (bulk
    OUT of `len(op.data)` bytes to EP 0x02; `data` = the captured chunk,
    kept as provenance/default -- docs/sane-hook4-shading.md section 6)

`Read`/`AckRead`/`PollDataReady`/`PollClass`/`BulkDone` carry the
captured reply as `data` (provenance, mirrored into a generated byte
blob); `Write`/`BulkOut` carry their payload the same way. `gl126_ops.cpp`
...
def emit() -> tuple[str, str]:
    ...
    h.append("};\n")
    h.append("/** One op-program transfer. `data` is the write payload (Write/BulkOut)")
    h.append(" *  or the captured reply (AckRead/Read/PollDataReady/PollClass/")
    h.append(" *  BulkDone, provenance only -- the runner does not require a live")
    h.append(" *  reply to match it, except AckRead's fixed 0x55, PollDataReady's")
    h.append(" *  bit 0x01 and PollClass's upper nibble); nullptr for BulkIn. */")
    h.append("struct Op {")
