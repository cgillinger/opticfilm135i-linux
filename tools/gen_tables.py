...
def main():
    ...
    w("")
    w("    Refuses a frame the holder does not have (of135i/holder.py). The")
    w("    bound lives here, at the one place a frame number becomes a motor")
    w("    target, so that no caller can position to a frame that has no")
    w("    aperture. Nothing has been written when it refuses.")
    w('    """')
    w("    holder.check_frame(frame)")
