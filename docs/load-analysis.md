...
## 5. Resolution (2026-09-05, after Test 14)

The hypothesis in §2/§4 — cassette pushed past the engagement point —
was wrong. Test 14's clean standalone vendor capture
(`traces/20260905-vendor-clean-load`, magazine taken fully out and
reinserted to the stop) shows the engaging feed programming the full
register block (four batches, 126 registers: 0x01=0x22, 0x03=0x30,
0x15=0x90, 0x35=0xbb, ...) right before the GO. Our table, cut from
the eject-from-loaded capture, ran the feed as a bare 19-register
batch on top of the base table (0x35=0xfb): the transport ran, the
cassette never engaged, at every insertion depth. Full insertion to
the stop IS the correct hand step.

Driver changes (offline, tests green, NOT hardware-verified):

- `tables_load.LOAD` regenerated by `tools/gen_load_table.py` from
  the clean-load trace, ops 194-260 (sensor ack, engaging feed with
  its register block, traverse, first idle housekeeping cycle).
  `tests/test_safety.py` checks byte identity against the trace and
  that the feed block carries the decisive registers.
- The strict exact-value polls (f055/d855) are replaced by the masked
  test `device.load_status_matches` (mask 0xfb): state class AND
  loader-sensor bit 0x08 AND busy bit; bit 0x04 (session-variable:
  f0/f4, d8/dc) ignored. Feed target f455 (sensor CLEAR), traverse
  target dc55 (sensor SET again); the final read must pass the same
  test. ec55, f855, f555, cc55, d455 all fail (tested).

Next hardware step: scanner power-cycled, magazine fully OUT, then
