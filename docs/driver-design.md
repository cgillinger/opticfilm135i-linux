...
| `device.py` | `Scanner` class: `initialize(ir, dpi)`, `cold_init()`, `eject()`, `is_magazine_present()`, `scan(frame, dpi=3600, ir=False)` returning the raw buffer; `_scan_dual(t)` drives any dual-light table module (tables_ir, tables_dpi<N>) |
...
- [x] Loader sensor and button events. Done 2026-09-02 (pass 16):
      ...
      addressing with wValue bit 0x100); `is_magazine_present()` checks
