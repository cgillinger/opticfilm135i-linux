...
scanner, where the full `of135i load` is required instead. Hardware-
verified once (docs/test-log.md Test 89): feed and traverse completed on
the first poll and the next frame positioned exactly like the strip
before it. Saves three motor moves and one remove-and-reseat per strip.
